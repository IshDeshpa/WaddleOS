...
// 12 byte header
struct __attribute((packed)) wdmalloc_hdr {
  uint16_t prev_sz;
  int16_t sz;
  union {
    wdmalloc_hdr_t *next;
...
static wdmalloc_hdr_t *freelist_head;
...
static void coalesce(wdmalloc_hdr_t *hdr){
  ...
  wdmalloc_hdr_t *next_nghbr = (wdmalloc_hdr_t *)((uintptr_t)hdr + sizeof(wdmalloc_hdr_t) + (hdr->sz & ~WDM_ALLOC_BIT));
