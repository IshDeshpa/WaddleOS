...
void setup_bitmap(){
  // Setup
  for(int i=0; i<(int)(sizeof(bmp_buf)/sizeof(uint8_t)); i++){
    bmp_buf[i] = 0xFF;
  }
  
  bitmap_init_buf(&bmp, bmp_buf, BMP_SIZE);
}

bool test_bitmap_init_buf(){
  // bitmap_init_buf initializes to 0
  for(int i=0; i<(int)(sizeof(bmp_buf)/sizeof(uint8_t)); i++){
    TEST_ASSERT_EQ_INT(bmp_buf[i], 0x0);
  }

  // Buf should be buf
  TEST_ASSERT_EQ_PTR(bmp.buf, bmp_buf);

  // Size should be correct
  TEST_ASSERT_EQ_LONG(bmp.size, BMP_SIZE);
...
bool test_bitmap_set(){
  for(int ind=0; ind<BMP_SIZE; ind++){
    ...
    int bitind = ind%64;
    int wordind = ind/64;
    uint8_t prev = bmp_buf[wordind];

    bitmap_set(&bmp, ind, false);
    TEST_ASSERT_EQ_INT(bmp_buf[wordind], prev & ~(1 << bitind));
  }
...
bool test_bitmap_get(){
  ...
  for(int ind=0; ind<BMP_SIZE; ind++){
    TEST_ASSERT_EQ_INT(bitmap_get(&bmp, ind), (bool)(ind%2));
  }
...
bool test_bitmap_flip(){
  ...
  for (int i = 0; i < (int)(sizeof(bmp_buf) / sizeof(uint8_t)); i++) {
    TEST_ASSERT_EQ_INT(bmp_buf[i], (uint8_t)0x55);
  }

  // Flip entire bitmap (again)
  bitmap_flip(&bmp, 0, BMP_SIZE-1);
  for (int i = 0; i < (int)(sizeof(bmp_buf) / sizeof(uint8_t)); i++) {
    TEST_ASSERT_EQ_INT(bmp_buf[i], (uint8_t)0xAA);
  }
  ...
  TEST_ASSERT_EQ_INT(bmp_buf[0], 0x55);

  // Other bytes should remain unchanged
  for (int i = 1; i < (int)(sizeof(bmp_buf) / sizeof(uint8_t)); i++) {
    TEST_ASSERT_EQ_INT(bmp_buf[i], 0xAA);
  }

  // Flip bits 4–11 (crosses byte boundary)
  bitmap_flip(&bmp, 4, 11);
  // Byte 0: bits 4–7 flipped → 0x55 -> bits 4–7 flip -> 0xA5
  TEST_ASSERT_EQ_INT(bmp_buf[0], 0xA5);
  // Byte 1: bits 0–3 flipped from 0xAA -> 0xA5
  TEST_ASSERT_EQ_INT(bmp_buf[1], 0xA5);
...
bool test_bitmap_test(){
  ...
  TEST_ASSERT_EQ_LONG(ret, -1); // no run of 1s exists 

  ret = bitmap_test(&bmp, false, 0, BMP_SIZE-1, 8);
  TEST_ASSERT_EQ_LONG(ret, 0); // first run of 8 0s is at index 0 (since everything is initialized to 0)
  
  bmp_buf[2] = 0xA3;

  ret = bitmap_test(&bmp, true, 0, BMP_SIZE-1, 2);
  TEST_ASSERT_EQ_LONG(ret, 16);
...
bool test_bitmap_test_and_flip() {
    ...
    TEST_ASSERT_EQ_LONG(ret, 0); // first run of 2 consecutive 1s

    // Verify the bits at index 16 and 17 were flipped to 0
    TEST_ASSERT_EQ_INT(0, (bmp_buf[0] >> 16) & 1);
    TEST_ASSERT_EQ_INT(0, (bmp_buf[0] >> 17) & 1);
