...
bool test_bitmap_init_buf(){
  ...
  for(int i=0; i<(int)(sizeof(bmp_buf)/sizeof(uint8_t)); i++){
    TEST_ASSERT_EQUAL_INT(bmp_buf[i], 0x0);
  }

  // Buf should be buf
  TEST_ASSERT_EQUAL_PTR(bmp.buf, bmp_buf);

  // Size should be correct
  TEST_ASSERT_EQUAL_SZ(bmp.size, BMP_SIZE);
...
bool test_bitmap_set(){
  for(int ind=0; ind<BMP_SIZE; ind++){
    ...
    TEST_ASSERT_EQUAL_INT(bmp_buf[wordind], prev & ~(1 << bitind));
  }
...
bool test_bitmap_flip(){
  ...
  for (int i = 0; i < (int)(sizeof(bmp_buf) / sizeof(uint8_t)); i++) {
    TEST_ASSERT_EQUAL_INT(bmp_buf[i], (uint8_t)0x55);
  }

  // Flip entire bitmap (again)
  bitmap_flip(&bmp, 0, BMP_SIZE-1);
  for (int i = 0; i < (int)(sizeof(bmp_buf) / sizeof(uint8_t)); i++) {
    TEST_ASSERT_EQUAL_INT(bmp_buf[i], (uint8_t)0xAA);
  }
...
bool test_bitmap_test_and_flip() {
    ...
    for (int i = 0; i < 8; i++) {
        TEST_ASSERT_EQUAL_INT(1, (bmp_buf[i / 64] >> (i % 64)) & 1);
    }
    ...
    TEST_ASSERT_EQUAL_INT(0, (bmp_buf[0] >> 16) & 1);
    TEST_ASSERT_EQUAL_INT(0, (bmp_buf[0] >> 17) & 1);
