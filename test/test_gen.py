...
class FuncDefVisitor(c_ast.NodeVisitor):
    ...
    def finalize(self, test_suite_name):
        ...
        test_suite_struct = (
            f'{{ "{test_suite_name}", '
            f'(test_t[]){{ {entries} }}, '
            f'{len(self.test_suite)} }}'
        )
        test_protos_str = "\n".join(self.test_protos)
        return test_protos_str, test_suite_struct
...
def print_func_defs(file, cpp_args):
    ...
    test_protos, test_suite_struct= v.finalize(test_name)

    return test_protos, test_suite_struct
...
if __name__ == "__main__":
    ...
    test_protos, test_suite_struct = print_func_defs(args.test_source, args.cflags[0].split(' '))
